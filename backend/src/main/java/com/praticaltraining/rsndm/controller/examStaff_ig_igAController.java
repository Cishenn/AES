...
public class examStaff_ig_igAController {
    ...
    @ResponseBody
    @CrossOrigin
    ResponseEntity<Map<String, List<examStaff_ig_igA>>> getInspectionTeamsAndPos(int eduId){
        Map<String,List<examStaff_ig_igA>> result = new HashMap<>();
        List<InvigilatorGroup> allInvigilatorGroup=invigilatorGroupBiz.getAllByEduId(eduId);
        ...
        for(int i=0;i<allInvigilatorGroup.size();i++){
            ...
            for(int j=0;j<allArrange.size();j++){
                ...
                examStaff_ig_igARes.setInvigilatorGroup(allInvigilatorGroup.get(i).getInvigilatorGroupId());
