          ...
          >
            ...
            <el-table-column align="center" label="考场审核状态" width="120px">
              <template slot-scope="scope">
                <el-tag
                  :type="
                    scope.row.exRoomExamine === 1
                      ? 'info'
                      : scope.row.exRoomExamine === 2
                      ? 'success'
                      : 'danger'
                  "
                  >{{
...
export default {
  ...
  methods: {
    ...
    deletefile (data) {
      // 删除文件
      setTimeout()
      this.$axios.get('/floor_exRoom/floor_exRoom/delete', {
        params: {
          path: 'C://uploadFile//' + data
        }
      }).then(resp => {
        alert(1)
      })
    ...
    downloadxls () {
      this.$axios.get('/floor_exRoom/floor_exRoom/outputAll/higherId', {
        params: {
          eduId: this.eduId
        }
      }).then(resp => {
        console.log(resp.data)
        this.data111 = 'http://localhost:8080/file/' + resp.data
        this.data222 = resp.data
        // setTimeout(this.deletefile(resp.data), 1000)
      })
    }
  }
}
